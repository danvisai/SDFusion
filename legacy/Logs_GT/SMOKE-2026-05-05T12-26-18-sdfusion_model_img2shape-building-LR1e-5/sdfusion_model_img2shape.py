...
class SDFusionImageFPShapeModel(BaseModel):
    ...
    def _build_fp3d_for(self, D, H, W, C=1):
        # Latent axes are (D=z, H=y, W=x) because the VQVAE 3D conv preserves the
        # (z,y,x) layout of the source SDF. The footprint h5 stores
        # (sdf<=0).any(axis=Y) — i.e. a (z,x) top-down silhouette — so it must be
        # placed on (D, W) and replicated along H. Earlier code put it on (H, W)
        # and replicated along D, which rotated the ground plan 90° around X.
        fp2d = self.fp  # [B, C?, fp_H, fp_W] where fp_H=z, fp_W=x
        ...
        fp2d_lat = F.interpolate(fp2d, size=(D, W), mode='nearest')   # [B,1,D,W]
        fp3d = fp2d_lat.unsqueeze(3).repeat(1, C, 1, H, 1)            # [B,C,D,H,W]
        return fp3d
