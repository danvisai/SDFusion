...
class SDFusionImageFPShapeModel(BaseModel):
    ...
    def inference(self, data, ddim_steps=None, ddim_eta=0., uc_scale=None, infer_all=False, max_sample=16):
       ...
       #uc   = uc_img + uc_fp
        ...
        uc = {
            'c_concat':   [fp3d],       # keep geometry constraint in both branches
    ...
    def img2shape(self, image, mask, ddim_steps=None, ddim_eta=0., uc_scale=None):
        """
        Args:
            image: PIL.Image or numpy array (RGB) - input image
            mask:  PIL.Image or numpy array (grayscale or binary) - footprint mask
        Returns:
            Generated SDF volume (self.gen_df)
        """
        ...
        # This assumes preprocess_image returns numpy arrays or PIL images
        _, img = preprocess_image(image, mask)  # returns masked image & original
        ...
        data = {
            'img': img,
            'fp': mask_tensor,
            'sdf': torch.zeros(1, 1, 64, 64, 64)  # dummy input
        }
