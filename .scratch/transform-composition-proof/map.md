...
- [Build the Full-Data Experiment Element Library](issues/08-build-full-data-element-library.md) - Extended ticket 04's `build_element_library.py` (no new script) with `crop_solidity`/`scale_rel`/`distribution_stats` (TDD, 9 new tests, 16/16 in the file); `crop_solidity` verified byte-identical to `element_fit.py`'s live `_solidity` formula. **Safety catch:** `scene/element_lib.py` serves the live sculptor from `data/element_library_v1/`, which predates the sealed split and has no leakage manifest at all — this build writes to a separate `data/element_library_train100_v1/`, never touching the production path (mtimes confirmed unchanged). Full `train_100` build (`--include-ids train_100.json --exclude-ids test.json`): **2744 elements from 1133/1572 buildings, 0 leakage**, 85.6% of the production 3204-element count vs. 85.0% of buildings selected (consistent, not a broken filter). New finding: `pool_size_above_min_solidity` is **0 for `stairs` and `balcony_upper`** at the existing `MIN_SOLIDITY=0.12` retrieval threshold — cross-checked as already true in the full 3204-element production library too, so it's a real, previously-unmeasured retrieval-pool gap (flagged for tickets 12/13's failure localization), not a bug in this build. `frozen_config` (RES=48, MIN_FACES=60, MAX_PER_TYPE_PER_BLDG=4, MAX_PER_TYPE=3000, the 8-type vocabulary) is recorded per-build in the manifest. Unblocks ticket 12.
- [Train the Full-Data Real-Pair Monolith](issues/11-train-full-data-monolith.md) - **v1/v2 negative, v3 follow-up succeeded — `monolith_v3` recommended for tickets 12/13.** Built a self-contained conditional 3D diffusion UNet (`models/networks/monolith_unet.py` + `models/monolith_diffusion.py`, raw 96³ voxels, not a new VQVAE) + resumable trainer (`scripts/foundations/train_monolith.py`, checkpoint/resume TDD-tested per the PRD's own training-smoke-test contract) + eval script, reusing ticket 07's pair-loading exactly. Caught and fixed a real DDIM `x0_pred` divergence bug (unclamped values outside [-16,7]; standard `clip_denoised` fix, regression-tested). Compute budget (15k steps, 1xA100, ~2.6h, 4.1M params) derived from a measured throughput smoke run, not guessed. **v1 (unweighted epsilon-prediction):** real BuildingNet targets are >90% constant background after truncation, so unweighted per-voxel MSE collapses to near-zero within ~2k steps while held-out DDIM sampling still shows **32.5% mean generated occupancy vs. 1.66% real** — the loss was converged on the trivial part of the volume. **v2 (surface-weighted, weight pre-registered from the measured 2.9% surface-band fraction):** made it **worse** (51.5% occupancy) — a genuine disclosed negative finding, naive loss reweighting doesn't fix a sparse-target diffusion process. **v3 (x0-prediction instead of epsilon-prediction, unweighted):** structural fix, not another weight guess — predicting x0 directly ties the loss to voxel sign more tightly and is more numerically stable at high noise. A 3k-step diagnostic already tracked real per-building occupancy closely; the full 15k-step run reached **mean generated occupancy 1.57% vs. 1.66% real** (median 0.06% vs 0.09%) — matched to within measurement noise, and the v1/v2 "every building decodes to the same blob" failure is visibly gone in the montage. Inference reproducibility passes cleanly on all three real checkpoints (DDIM eta=0, bit-identical same-seed). **Recommendation: `monolith_v3` is the reference checkpoint for tickets 12/13** — occupancy-fraction match is a necessary but not sufficient proxy for "strong" (says nothing about facade/geometric fidelity, which is ticket 13's own FID/paired-metric protocol to establish), but it passes this ticket's sanity bar where v1/v2 explicitly did not.
- [Prototype the Sculpt Strength Sweep](issues/10-prototype-sculpt-strength-sweep.md) - Swept the canonical tower/dome/carve edits × strengths 0.1-0.9 through the live `/snap_sdf` operator (in-process `Refiner.snap_volume`, all other params at production defaults). First a procedural recipe box (too flat to review), then one real building, then scaled to all 27 Stage3a-clean held-out buildings × 3 edits = 81 distinct shapes (2268 pooled images) + 200 real reference buildings from `train_100.json` (5600 images) so `fid.undersampled` finally reads **false** at every strength. But the bootstrap point estimate still falls *outside* its own CI at every strength — the group-aware bootstrap's effective N is the 81/200 distinct-shape count, not the view-inflated image count, so this isn't fully trustworthy yet either; disclosed, not hidden. Also newly quantified: **56% of the 27-building held-out population has occupancy <0.5%** (median 0.31%) — a more systematic characterization than ticket 09's own "0.02%-6.4%, no natural gap" note, relevant to any other ticket reusing this same population. Caught and fixed a real confound before the final run: `sdedit`'s one unseeded noise draw meant strength wasn't actually the sole controlled variable; fixed via a reset seed per call. Artifact built for user review, not a closed decision — see the ticket's own "Left for the user's review" note.

## Pivot (2026-07-12)

**Tickets 12/13 (decomposition arm, C2 kill-gate) are PAUSED** — project-owner decision after
ticket 11 closed, not a blocker or failure. Reasoning: geometric facade *detail* (what C1/C2 is
about) is lower priority right now than base massing/shape quality, since appearance/texture is
handled by a separate layer. A user-run diagnostic (not a tracked ticket —
`scripts/eval/diagnose_massing_diversity.py`, `outputs/massing_diversity/`) confirmed
from-noise massing generation is lumpy/blobby (replicating ticket 09's own finding) while the
production SDEdit-blockout path gives clean rectilinear shapes at a real, measured diversity cost
(0.537 mean pairwise-IoU-diversity from-noise vs 0.378 via SDEdit, k=8 samples over 8 held-out
footprints). This directly motivates resuming **ticket 10** (sculpt strength sweep) next — it was
already unblocked (needs 01, 05, both done early) but got skipped over when ticket 11 was picked
up instead, deviating from `execution/IMPLEMENTATION_PLAN_2026-07-10.md`'s own prescribed order
("M-C1 sculpt sweep... before any training"). Resume tickets 12/13 only after checking with the
project owner whether priorities have shifted back — see the memory file
`project-priority-pivot-2026-07-12-massing-over-c2.md` for full context.

## Not yet specified

- Whether the 27-building Stage3a-clean held-out population's sparsity (ticket 10: 56% below 0.5%
  occupancy, median 0.31%) should change how any ticket reusing this population (09 already has,
  future ones might) reports or filters results — no ticket has yet decided whether to separate
  the sparse/near-empty regime from the well-formed one, or continue pooling them.
