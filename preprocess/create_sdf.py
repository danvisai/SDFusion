...
def bake_footprint(mesh_file, resolution):
    mesh = trimesh.load_mesh(mesh_file, process=False)
    
    verts = mesh.vertices[:, [0,2]]  # only X,Y
    # shift & scale to [0,1]²
    min_xy = verts.min(axis=0)
    max_xy = verts.max(axis=0)
    span = max_xy - min_xy
    norm = (verts - min_xy) / span
    # build an approximate “footprint polygon” by projecting each triangle:
    footprint = np.zeros((resolution, resolution), dtype=np.uint8)
    for tri in mesh.faces:
        tri_xy = norm[tri]  # shape (3,2)
        # rasterize that triangle into your grid:
        # we can use a simple bounding‐box fill for speed
        min_b = np.floor(tri_xy.min(axis=0) * (resolution-1)).astype(int)
        max_b = np.ceil(tri_xy.max(axis=0) * (resolution-1)).astype(int)
        min_b = np.clip(min_b, 0, resolution-1)
        max_b = np.clip(max_b, 0, resolution-1)
        for i in range(min_b[1], max_b[1]+1):
            for j in range(min_b[0], max_b[0]+1):
                # barycentric test
                p = np.array([j/(resolution-1), i/(resolution-1)])
...
def create_h5_sdf_pt(h5_file, sdf_file, flag_file, norm_obj_file,
    ...
    #new added code for footprint generation
    resolution  = sdf_res // reduce
    footprint = bake_footprint(norm_obj_file,resolution)
    
    #below three lines are sanity checks for footprints
    print(f"[DEBUG] Baking footprint for {h5_file}:")
    print(f"         footprint.shape = {footprint.shape}, unique={np.unique(footprint)}")
    print(f"         # of occupied pixels = {footprint.sum()}")

    f1.create_dataset('footprint', data = footprint[np.newaxis,...],compression = 'gzip', compression_opts = 4)
    print(f"[DEBUG] Written footprint ds shape = {f1['footprint'].shape}")
...
if __name__ == "__main__":
    ...
    # pick the correct info-<dset>.json for shapenet / pix3d / building / abc
    info_dir  = os.path.join(CUR_PATH, '..', 'dataset_info_files')
    info_file = os.path.join(info_dir, f'info-{FLAGS.dset}.json')
    print(f"[create_sdf] loading dataset info from {info_file}")
    with open(info_file, 'r') as f:
        info_data = json.load(f)

    # for building we only need raw_dirs_v1
    raw_dirs = info_data['raw_dirs_v1']
    print("[create_sdf] raw_dirs =", json.dumps(raw_dirs, indent=2))


# for ShapeNet we also need lst_dir, cats, all_cats
    if FLAGS.dset == 'shapenet':
        lst_dir, cats, all_cats = (
        info_data['lst_dir'],
        info_data['cats'],
        info_data['all_cats']
    )
