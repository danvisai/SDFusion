...
## Clause kinds
  ...
  * **KILL** -- the structural capability #188 exists to restore. Any failure is a KILL verdict, and
    is not outvoted by the clauses that passed (#183's own rule, per #172(b)).
  * **BAR** -- the non-inferiority clauses above. Failure is NOT MET, not a KILL.
  * **INFORMATIONAL** -- reported, never scored. The legacy pinned-714 regression control lives
    here because [#119](https://github.com/danvisai/SDFusion/issues/119) ruled that no gate verdict
    is scored on the pinned 714.
"""
...
def build_registration(reference: dict, frozen_ratios: dict, operating_point: dict,
    ...
    **before any retrained checkpoint existed** -- they are properties of the data, so measuring
    ...
    clauses = [
        ...
        {"id": "K3", "kind": "KILL", "source": "summary", "arm": arm, "metric": "fp_iou",
         "rule": ">=", "threshold": 0.90,
        ...
        {"id": "K4", "kind": "KILL", "source": "summary", "arm": arm, "metric": "collapse_rate",
         "rule": "<=", "threshold": 0.25,
         "rationale": "Hollow collapse (>=15% of GT volume missing) is this family's known failure "
                      "mode: #92's aligned retrain reached 0.4636 and was recorded as a failure, "
                      "while the frozen source measured 0.1667. 0.25 sits between the two and is "
                      "fixed before any retrained number is seen."},
        {"id": "B1", "kind": "BAR", "source": "ratio", "arm": arm, "metric": "vol_iou",
        ...
        {"id": "B2", "kind": "BAR", "source": "ratio", "arm": arm, "metric": "extra",
        ...
        {"id": "B3", "kind": "BAR", "source": "summary", "arm": arm, "metric": "missing",
         "rule": "<=", "threshold": 0.05,
        ...
        {"id": "I1", "kind": "INFORMATIONAL", "source": "summary", "arm": arm, "metric": "vs_input",
         "rule": "<=", "threshold": 0.99,
         "rationale": "Reported, never scored. `vs_input` near 1.0 is the near-no-op signature #87 "
                      "measured on the frozen source (median 0.9882/0.9832/0.9813 by region). "
                      "Worth seeing on the new corpus; not a verdict, because no arm in this "
                      "family has ever cleared it."},
    ]
    return {
        "meta": {
            "ticket": 188,
...
def score_registration(summary: dict, registration: dict, arm: str, extras: dict) -> dict:
    ...
    scored, kill_clear, bars_met = [], True, True
    for clause in registration["clauses"]:
        ...
        if resolved["kind"] == "KILL" and not ok:
            kill_clear = False
        if resolved["kind"] == "BAR" and not ok:
            bars_met = False
        scored.append({**resolved, "value": value, "pass": bool(ok), "measured": True})

    verdict = "KILL" if not kill_clear else ("PASS" if bars_met else "NOT MET")
    return {"arm": arm, "verdict": verdict, "clear_kill": kill_clear, "numeric_pass": bars_met,
            "clauses": scored}
...
def main() -> None:
    ...
    sc.add_argument("--generation_failures", type=int, default=0)
    sc.add_argument("--buckets_scored", type=int, required=True)
    ...
    verdict = score_registration(
        artifact["summary"], registration, arm=args.arm,
        extras={"generation_failures": args.generation_failures,
                "buckets_scored": args.buckets_scored})
    print(f"=== #188 bar: {verdict['verdict']} "
          f"(clear_kill={verdict['clear_kill']}, numeric_pass={verdict['numeric_pass']}) ===")
    for clause in verdict["clauses"]:
        mark = "PASS" if clause["pass"] else "FAIL"
        print(f"  {clause['id']:>3} {clause['kind']:<13} {clause['metric']:<18} "
              f"{clause['value']:>9.4f} {clause['rule']} {clause['threshold']:<8} {mark}")
    if args.out:
