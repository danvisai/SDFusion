...
class SDFusionModel(BaseModel):
    ...
    def initialize(self, opt):
        ...
        print(df_conf)
        unet_params = df_conf.model.unet.params
    ...
    def save(self, label, global_step, save_opt=True):
        ...
        self.cleanup_old_ckpts(self.opt.ckpt_dir, max_keep=3, min_free_gb=5)
    ...
    def load_ckpt(self, ckpt, load_opt=False):
        ...
        if load_opt:
            #self.optimizer.load_state_dict(state_dict['opt']) #old code 
            
            #print(colored('[*] optimizer successfully restored from: %s' % ckpt, 'blue')) #old code

            if 'opt' in state_dict:         #new code
                self.optimizer.load_state_dict(state_dict['opt'])                              #new code
                print(colored('[*] optimizer successfully restored from: %s' % ckpt, 'blue')) #new code
            else:                                                                                                   #new code
                print(colored('[*] no optimizer state in checkpoint – starting with a fresh optimizer','yellow'))   #new code



    def cleanup_old_ckpts(self,ckpt_dir, max_keep=5, min_free_gb=10):
        # 1) Delete old files if disk is getting full
        total, used, free = shutil.disk_usage(ckpt_dir)
        free_gb = free / (1024**3)
        if free_gb < min_free_gb:
            print(f"[cleanup] only {free_gb:.1f} GB free, pruning old checkpoints…")
    # 2) Or just keep last N
        ckpts = sorted(glob.glob(os.path.join(ckpt_dir, "df_*.pth")),
                   key=os.path.getmtime)
        to_remove = ckpts[:-max_keep]
        for path in to_remove:
            print(f"[cleanup] removing old checkpoint {os.path.basename(path)}")
            os.remove(path)
