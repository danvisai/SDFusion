...
class SDFusionImage2ShapeModel(BaseModel):
    ...
    def initialize(self, opt):
        ...
        # setup renderer
        # dist, elev, azim = 1.7, 20, 20   #old code
        # self.renderer = init_mesh_renderer(image_size=256, dist=dist, elev=elev, azim=azim, device=self.opt.device) #old code
        if not self.isTrain:
    ...
    def set_input(self, input=None, gen_order=None, max_sample=None):
        ...
        #self.fp = input['fp'] #new code 

        self.uc_img = torch.zeros_like(self.img).to(self.device)
        #self.uc_fp = torch.zeros_like(self.fp).to(self.device)  #new code

        if max_sample is not None:
            self.x = self.x[:max_sample]
            self.img = self.img[:max_sample]
            self.uc_img = self.uc_img[:max_sample]
            #self.fp = self.fp[:max_sample]      #new code
            #self.uc_fp = self.uc_fp[:max_sample] #new code

        vars_list = ['x', 'img'] #old code commented
        #vars_list = ['x','img','fp','uc_img','uc_fp']

        self.tocuda(var_names=vars_list)
        #self.fp = self.fp.to(self.device) #new code
        #self.uc_fp = self.uc_fp.to(self.device) #new code
    ...
    def forward(self):
        ...
        #c_fp = self.cond_model(self.fp).float() # new code
        ...
        z_noisy, target, loss, loss_dict = self.p_losses(z, c_img, t) #old code commented
        #z_noisy, target, loss, loss_dict = self.p_losses(z, [c_img, c_fp], t) #new code
        
       
        # _, _, loss_with_fp, _ = self.p_losses(z, [c_img, c_fp], t) #new Code

        # # --- compute loss with footprint zeroed out ---    new code
        # zeros_fp = torch.zeros_like(self.fp)                #new code
        # c_fp_zero = self.cond_model(zeros_fp).float()       #new code
        # _, _, loss_no_fp, _ = self.p_losses(z, [c_img, c_fp_zero], t)   #new code

        # print them so you can see in your console
        # print(f"[sanity]  loss_with_fp = {loss_with_fp.item():.4f},  loss_no_fp = {loss_no_fp.item():.4f}") #new code
    ...
    def inference(self, data, ddim_steps=None, ddim_eta=0., uc_scale=None,
        ...
        uc = self.cond_model(self.uc_img).float() # img shape #commented old code
        # uc_img = self.cond_model(self.uc_img).float()
        # uc_fp  = self.cond_model(self.uc_fp).float()
        c_img = self.cond_model(self.img).float()
        #c_fp   = self.cond_model(self.fp).float()
        ...
        samples, intermediates = self.ddim_sampler.sample(S=ddim_steps,
                                                        batch_size=B,
                                                        shape=shape,
                                                        conditioning= c_img,  #[c_img, c_fp], #new code
                                                        verbose=False,
                                                        unconditional_guidance_scale=uc_scale,
                                                        unconditional_conditioning=uc,#[uc_img, uc_fp], #new code
                                                        eta=ddim_eta,
                                                        quantize_x0=False)
